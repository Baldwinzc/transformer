...
class Encoder(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, dropout = 0.1):
        super().__init__() # 修正: super.__init__()
        self.layernorm = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(2)])
        self.atten = MultiHeadAttention(d_model,num_heads,dropout)
        self.ffn = FeedForward(d_model,d_ff, dropout)
        self.dropout = nn.Dropout(dropout)
    
    def forward(self, x, mask): # (batch_size, seq_len, d_model)
        x_norm = self.layernorm[0](x)
        atten_output = self.atten(
            x_norm,
            x_norm,
            x_norm,
            mask
        ...
        # 修正: self.layernorm[1](x) 括号位置错误
        ffn_output = self.ffn(self.layernorm[1](x))

        x = x + self.dropout(ffn_output)
        return x # 修正: 加上 return x
